...
class ParseMessage:
    ...
    def clear_path(self):
        self.current_branch = ""
        self.message = ""
        self.parser.clear_path()

    def find_path(self, path_temp):
        # we now check for instance of the path in the dictionary
        err_short = self.parser.curr_dic_short(str(path_temp).upper())
        err_long = self.parser.curr_dic_long(str(path_temp).upper())
    ...
    def find_in_common(self, path_temp):
        error = self.parser.common(str(path_temp).upper())
        if not error:
            self.response = "Wrong path, problem with: (No such directory) - " + str(path_temp) + "Try again\n"
            self.message = ""
        return error
